...
function renderPreview(summary,label='M3U preview'){
  ...
  for(const name of summary.sample){
    const chip = document.createElement('span'); chip.textContent = name; chips.appendChild(chip);
  }
...
async function fetchWithTimeout(url,timeout=9000){
  ...
  try{return await fetch(url,{cache:'no-store',signal:c.signal});}finally{clearTimeout(t);}
}
...
async function fetchPlaylist(url){
  ...
  try{
    ...
    if(r.ok){
      ...
      if(summarize(text).count) return {text,via:'direct'};
    }
  }catch{}
...
async function applyText(text,mode,label){
  ...
  setStatus(`${label} loaded · ${result.imported} channels · ${mode}`,'ok');
  log(`PLAYLIST ${mode.toUpperCase()} · ${label} · ${result.imported} imported · ${result.total} total`);
  return result;
}
...
async function testUrl(){
  ...
  try{
    ...
    const summary = summarize(fetched.text);
...
async function loadTemporary(kind){
  try{
    ...
    const label = kind==='url' ? (p.url||'External URL') : 'Pasted M3U';
...
async function saveCurrent(kind){
  try{
    setStatus('Saving playlist…','busy');
    const p = await currentPayload(kind);
    ...
    setStatus(`${name} saved · ${p.summary.count} channels`,'ok');
    log(`PLAYLIST SAVED · ${name} · ${p.summary.count} channels`);
    await renderSaved();
  }catch(error){setStatus(error.message,'error');}
}

function downloadM3U(item){
  const blob = new Blob([item.text],{type:'audio/x-mpegurl;charset=utf-8'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `${item.name.replace(/[^a-z0-9α-ω_-]+/gi,'_')||'playlist'}.m3u`;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}

async function makeActive(item,mode){
  setActiveState({id:item.id,mode});
  await applyText(item.text,mode,item.name);
  await renderSaved();
  setStatus(`${item.name} is now active · ${mode}`,'ok');
}
...
async function useRepo(){
  ...
  const bridge = api();
  if(!bridge) return;
  const result = bridge.resetToRepo();
  setStatus(`Repo Playlist active · ${result.total} channels`,'ok');
  log(`PLAYLIST ACTIVE · Repo Playlist · ${result.total} channels`);
  await renderSaved();
}

function tinyButton(label,cls='ghost'){
  const b=document.createElement('button'); b.type='button'; b.className=`button ${cls} mini`; b.textContent=label; return b;
}

async function renderSaved(){
  const box = $('saved-playlists');
  const count = $('saved-playlist-count');
  if(!box) return;
  ...
  repo.className = `playlist-card builtin${!active?' active':''}`;
  repo.innerHTML = `<div class="playlist-card-icon">⌂</div><div class="playlist-card-main"><div class="playlist-card-title"><strong>Repo Playlist</strong>${!active?'<span class="playlist-active-badge">ACTIVE</span>':''}</div><span>Built-in fallback · GitHub repository</span></div>`;
  const repoActions=document.createElement('div');repoActions.className='playlist-card-actions';const repoLoad=tinyButton('Use','ghost');repoLoad.addEventListener('click',useRepo);repoActions.append(repoLoad);repo.append(repoActions);box.appendChild(repo);

  for(const item of rows){
    const isActive=active?.id===item.id;
    const card=document.createElement('article');card.className=`playlist-card${isActive?' active':''}`;
    ...
    const title=document.createElement('div');title.className='playlist-card-title';const strong=document.createElement('strong');strong.textContent=item.name;title.appendChild(strong);if(isActive){const badge=document.createElement('span');badge.className='playlist-active-badge';badge.textContent=`ACTIVE · ${active.mode}`;title.appendChild(badge);}
    const meta=document.createElement('span');meta.textContent=`${item.channelCount||0} channels · ${item.groupCount||0} groups${item.url?` · ${new URL(item.url).hostname}`:''}`;main.append(title,meta);
    ...
    const activeBtn=tinyButton('Set Active','ghost');activeBtn.addEventListener('click',()=>makeActive(item,selectedMode()));
    const rename=tinyButton('Rename','ghost');rename.addEventListener('click',async()=>{const name=prompt('Playlist name',item.name);if(!name?.trim())return;await putSaved({...item,name:name.trim(),updatedAt:Date.now()});await renderSaved();});
    const exportBtn=tinyButton('Export','ghost');exportBtn.addEventListener('click',()=>downloadM3U(item));
    const del=tinyButton('Delete','danger');del.addEventListener('click',async()=>{if(!confirm(`Delete “${item.name}”?`))return;await removeSaved(item.id);if(active?.id===item.id){setActiveState(null);api()?.resetToRepo();}await renderSaved();setStatus(`${item.name} deleted`,'idle');});
    actions.append(load,activeBtn,rename,exportBtn,del);card.append(icon,main,actions);box.appendChild(card);
  }
}
...
async function restoreActive(){
  const active=activeState();
  if(!active) return;
  const item=await getSaved(active.id);
  if(!item){setActiveState(null);return;}
...
function bind(){
  $('playlist-manager-toggle')?.addEventListener('click',()=>{const p=$('playlist-manager');if(p)p.hidden=!p.hidden;});
  ...
  $('playlist-use-repo')?.addEventListener('click',useRepo);
...
window.addEventListener('webtv:ready',()=>restoreActive().then(renderSaved).catch(error=>setStatus(error.message,'error')),{once:true});
if(window.WebTVPlaylistAPI?.ready) restoreActive().then(renderSaved).catch(()=>{});
log(`Playlist Manager loaded · build ${BUILD_ID}`);
