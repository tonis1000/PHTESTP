...
function normalize(v=''){return String(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9α-ω]+/gi,'-').replace(/^-+|-+$/g,'');}
...
async function pull(){
  ...
  if(!token())throw new Error('Admin token is required to sync Saved Playlists');
  status('Pulling protected D1 library…','busy');
  const my=await api('/api/my-playlist');
  const myText=channelsToM3U(my.channels||[]);
  await put({id:MY_ID,name:'My Playlist',type:'curated',url:'',text:myText,channelCount:(my.channels||[]).length,groupCount:new Set((my.channels||[]).map(c=>c.groupName||'Other')).size,createdAt:Date.now(),updatedAt:Date.now()});

  const list=await api('/api/playlists');
  let saved=0;
  for(const meta of list.playlists||[]){
    const detail=(await api(`/api/playlists/${encodeURIComponent(meta.id)}`)).playlist;
    if(!detail?.rawM3u)continue;
    await put({id:detail.id,name:detail.name,type:detail.kind||'saved',url:detail.sourceUrl||'',text:detail.rawM3u,channelCount:detail.channelCount||0,groupCount:detail.groupCount||0,createdAt:Date.parse(detail.createdAt)||Date.now(),updatedAt:Date.parse(detail.updatedAt)||Date.now()});
    saved++;
  }
  status(`D1 pull complete · ${(my.channels||[]).length} My Playlist channels · ${saved} saved playlists`,'ok');
  const panel=$('playlist-manager');
  if(panel&&!panel.hidden){$('playlist-manager-toggle')?.click();setTimeout(()=>$('playlist-manager-toggle')?.click(),40);}
}

const button=$('registry-sync-down');
if(button){button.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();pull().catch(error=>status(`D1 pull failed · ${error.message}`,'error'));},true);}
