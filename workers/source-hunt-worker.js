...
function parseM3u(text='',channel=''){
  ...
  for(let i=0;i<lines.length;i++){
    const extinf=lines[i].trim();if(!/^#EXTINF:/i.test(extinf)||!relevant(extinf,channel))continue;
    let stream='';for(let j=i+1;j<Math.min(lines.length,i+10);j++){const next=lines[j].trim();if(!next||next.startsWith('#'))continue;if(/^https?:\/\//i.test(next))stream=cleanUrl(next);break;}
...
async function redditSearch(channel,budget){
  if(!budget.canUse())return [];budget.take();
  const p=profile(channel),q=`${p.searches[0]||channel} m3u8 IPTV stream playlist`;
  const u=new URL('https://www.reddit.com/search.json');u.searchParams.set('q',q);u.searchParams.set('sort','new');u.searchParams.set('t','month');u.searchParams.set('limit','15');u.searchParams.set('raw_json','1');
  try{const r=await timedFetch(u,{headers:{'user-agent':'WebTV-SourceHunt/1.12 (+public stream discovery)'}},BRAVE_TIMEOUT_MS);if(!r.ok)return [];const j=await r.json();return (j?.data?.children||[]).map(x=>x?.data).filter(Boolean).map(d=>({title:d.title||'',description:d.selftext||'',url:d.url_overridden_by_dest||`https://www.reddit.com${d.permalink||''}`,permalink:d.permalink?`https://www.reddit.com${d.permalink}`:'',updatedAt:d.created_utc?new Date(d.created_utc*1000).toISOString():null,_kind:'forum',_reddit:true})).filter(r=>freshEnough(r,30));}catch{return [];}
}
...
async function scanSeed(seed,channel,budget,debug){
  const out=[],report={type:'seed',name:seed.name,status:null,accepted:0};if(!budget.canUse())return out;const f=await fetchText(seed.url,budget);report.status=f.status;
  if(f.ok)for(const e of parseM3u(f.text,channel)){let u=e.url,method='extinf-seed';if(isStrm(u)){const rr=await resolveStrm(u,budget);if(!rr)continue;u=rr;method='extinf-seed-strm';}if(isLiveUrl(u))addUnique(out,candidate(u,'seed','Known Greek M3U seed',seed,{method,extinf:e.extinf}));}
...
async function inspectResult(r,channel,budget,debug,{strict=false}={}){
  ...
  if(!freshEnough(r,30)){report.rejected='stale';if(debug)debug.push(report);return {candidates,leads};}
  for(const u of extractLive(context)){
    const proven=urlCarriesChannel(u,channel)||(!strict&&tvEvidence(`${context} ${u}`,channel));
    if(proven&&!REJECT_VARIANTS.test(`${context} ${u}`)&&!(channel==='ANT1'&&ANT1_SUBCHANNELS.test(context)))addUnique(candidates,candidate(u,kind,origin,r,{method:'snippet',provenance:urlCarriesChannel(u,channel)?'channel-in-url':'page-context'}));
  }
  for(const u of extractUsefulLinks(context))if(!isLiveUrl(u)&&!explicitOldDate(u))addUnique(leads,lead(u,kind,origin,r,{method:'linked-lead'}));
  if(!candidates.length&&r.url&&relevant(context,channel)&&!LOW_TRUST_HOSTS.test(hostOf(r.url)))addUnique(leads,lead(r.permalink||r.url,kind,origin,r,{method:r._reddit?'reddit-post':'source-page'}));
  if(!candidates.length&&r.url&&budget.canUse()){
    const f=await fetchText(toRawGithubUrl(r.url),budget);report.status=f.status;
    if(f.ok){
      for(const e of parseM3u(f.text,channel)){let u=e.url;if(isStrm(u)){const rr=await resolveStrm(u,budget);if(!rr)continue;u=rr;}if(isLiveUrl(u))addUnique(candidates,candidate(u,kind,origin,r,{method:'extinf-page',extinf:e.extinf,provenance:'exact-extinf'}));}
      for(const u of extractLive(f.text)){
        if(candidates.some(x=>x.url===u))continue;const idx=f.text.indexOf(u),near=idx>=0?f.text.slice(Math.max(0,idx-500),Math.min(f.text.length,idx+u.length+500)):'';
        const proven=urlCarriesChannel(u,channel)||(!strict&&tvEvidence(`${context} ${near} ${u}`,channel));
        if(proven&&!REJECT_VARIANTS.test(`${context} ${near} ${u}`)&&!(channel==='ANT1'&&ANT1_SUBCHANNELS.test(`${context} ${near}`)))addUnique(candidates,candidate(u,kind,origin,r,{method:'nearby',provenance:urlCarriesChannel(u,channel)?'channel-in-url':'nearby-context'}));
      }
      for(const u of extractUsefulLinks(f.text))if(!isLiveUrl(u)&&!explicitOldDate(u))addUnique(leads,lead(u,kind,origin,r,{method:'page-lead'}));
    }
  }
...
async function inspectLead(url,channel){
  const budget=new Budget(5),source={title:'Lead inspection',url,description:'',updatedAt:new Date().toISOString()};
  const r=await inspectResult({...source,_kind:/reddit\.com|forum|thread/i.test(url)?'forum':'web'},channel,budget,null,{strict:true});
...
async function runHunt(env,channel,days,wantDebug=false){
  ...
  const seedRuns=await Promise.all(SEEDS.map(s=>scanSeed(s,channel,budget,wantDebug?debug:null)));for(const rows of seedRuns)for(const c of rows)addUnique(seed,c);
  const queries=buildQueries(channel);
  const tasks=[...queries.map(x=>brave(env,x.q,budget,8).then(rows=>({x,rows:rows.filter(r=>freshEnough(r,days))})).catch(error=>({x,rows:[],error}))),redditSearch(channel,budget).then(rows=>({x:{kind:'forum',q:'reddit-json'},rows}))];
  const queryRuns=await Promise.all(tasks),merged=[];let searches=0;
...
export default{async fetch(request,env){
  ...
  if(url.pathname!=='/hunt')return json({ok:true,service:'WebTV Source Hunt Worker',version:VERSION,features:['provenance guard','strict inspect','stale lead rejection','direct Reddit JSON search','Web/Forum leads','strict ANT1 filter','parallel discovery','15m cache'],endpoint:'/hunt?channel=SKAI&days=30'});
