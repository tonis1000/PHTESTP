...
function text(data,status=200,type='text/plain;charset=utf-8',origin='*'){
  return new Response(data,{status,headers:{...cors(origin),'content-type':type,'cache-control':'no-store'}});
}

function clean(value=''){return String(value??'').trim();}
function escAttr(value=''){return clean(value).replace(/"/g,"'");}
function normalizeId(value=''){
  return clean(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9α-ω]+/gi,'-').replace(/^-+|-+$/g,'').slice(0,160);
}
...
function requireAdmin(request,env){
  if(!env.ADMIN_TOKEN) return {ok:false,response:json({error:'ADMIN_TOKEN is not configured'},503,requestOrigin(request,env))};
  const auth=request.headers.get('authorization')||'';
  const token=auth.replace(/^Bearer\s+/i,'').trim();
  if(!token||token!==env.ADMIN_TOKEN) return {ok:false,response:json({error:'Unauthorized'},401,requestOrigin(request,env))};
  return {ok:true};
}
...
async function listPlaylists(env){
  const r=await env.DB.prepare(`SELECT id,name,kind,source_url AS sourceUrl,channel_count AS channelCount,group_count AS groupCount,created_at AS createdAt,updated_at AS updatedAt FROM playlists ORDER BY updated_at DESC`).all();
  return r.results||[];
}
...
async function myPlaylist(env){
  ...
  const by=new Map();
  for(const s of sourcesResult.results||[]){if(!by.has(s.channelId))by.set(s.channelId,[]);by.get(s.channelId).push(s);}
...
async function putMyChannel(env,payload){
  ...
  const logo=clean(payload.logo);
  const groupName=clean(payload.groupName||payload.group)||'Other';
...
function toM3u(channels){
  ...
  for(const c of channels){
    ...
    if(!sources.length){
      lines.push(`#EXTINF:-1 tvg-id="${escAttr(c.tvgId||c.id)}" tvg-name="${escAttr(c.name)}" tvg-logo="${escAttr(c.logo)}" group-title="${escAttr(c.groupName||'Other')}",${c.name}`);
      lines.push('');
      continue;
    }
...
export default {
  async fetch(request,env){
    ...
    try{
      ...
      if(path==='/playlist.m3u'&&request.method==='GET'){
        return text(toM3u(await myPlaylist(env)),200,'audio/x-mpegurl;charset=utf-8',origin);
      }
      ...
      if(path.startsWith('/api/playlists/')&&request.method==='DELETE'){
        const auth=requireAdmin(request,env);if(!auth.ok)return auth.response;
        const id=decodeURIComponent(path.slice('/api/playlists/'.length));
        await env.DB.prepare(`DELETE FROM playlists WHERE id=?`).bind(id).run();
        return json({ok:true,id},200,origin);
      }

      if(path==='/api/my-playlist'&&request.method==='GET'){
        const channels=await myPlaylist(env);
        return json({channels,count:channels.length,playlistUrl:`${url.origin}/playlist.m3u`},200,origin);
      }
      ...
      if(path.startsWith('/api/my-playlist/channel/')&&request.method==='DELETE'){
        const auth=requireAdmin(request,env);if(!auth.ok)return auth.response;
        const id=decodeURIComponent(path.slice('/api/my-playlist/channel/'.length));
        await env.DB.prepare(`DELETE FROM my_playlist WHERE channel_id=?`).bind(id).run();
        return json({ok:true,id},200,origin);
      }
      ...
      if(path==='/api/my-playlist/replace'&&request.method==='POST'){
        ...
        if(!Array.isArray(body.channels)) return json({error:'channels array is required'},400,origin);
        await env.DB.prepare(`DELETE FROM my_playlist`).run();
        let position=0;
        for(const channel of body.channels) await putMyChannel(env,{...channel,position:position++,replaceSources:true});
        return json({ok:true,count:body.channels.length},200,origin);
      }
